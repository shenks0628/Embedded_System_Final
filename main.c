...
void main(void) {
    ...
    while (1) {
		key = KeyPro();
        if (rec_flag == 1) {// 有UART輸入
            for (i = 0;i < 8;j++) TempData[i] = 0; //清屏
            buf[head] = '\0';
			if(buf[0]=='W'){// 猜對
				//顯示True
				state=PREPARE;
			...
			else if(buf[0]>='0' && buf[0]<='6'){//得到我的數字
				//顯示我的數字
				state=WAIT;
			}
		...
		if(key!=0xff){// 有鍵盤輸入
			if(state==GUESS){// 猜測
				if(key>=1 && key<=10){
					...
					UART_SendStr(strcat(itos(guess_cnt),itos(guess_num)));//傳送猜測
					state=WAIT;
				}
